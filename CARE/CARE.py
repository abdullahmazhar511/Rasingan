...
# Cache directory for the generated per-sample explanation JSONs.
DEFAULT_RAG_DIR = "./rag_cache"
...
# Create directories if they don't exist — only when run as a script.
# (Importing CARE from score_with_care.py used to pollute the importer's CWD.)
...
# CSVs use abbreviated trait column names; CONFIG["labels"] uses full names.
# Rename on read so downstream code finds the columns it expects.
COLUMN_MAP = {
...
def prepare_context(df, m=3):
    ...
    # str(x) inside the lambda is defensive against pandas 3.0 PyArrow-backed
    # string arrays that keep NaN as float instead of converting to 'nan'.
    df['ConvID'] = df['ID'].apply(lambda x: str(x).split('_')[0])
    contexts = []
...
def generate_explanations_for_split(df, split_name, ideal_sets, train_embeddings, train_df_ref, output_path):
    ...
    # batch 256 OOMed on verl's transformers 4.56.2 (MLP intermediate at batch*seq*intermediate_size
    # exceeds ~8 GB per layer). 128 with flash-attn is the safe spot on H200 (141 GB).
    batch_size = 128
    generated_texts = []
...
class QwenHierarchicalClassifier(nn.Module):
    def __init__(self, model_id, class_weights=None, binary_weights=None):
        ...
        # Gradient checkpointing: trade ~30% extra compute for ~10x activation-memory savings.
        # Needed because batch 16 x seq 1536 on Qwen3-4B materializes >100 GB of attention activations.
...
def evaluate():
    ...
    print("\nResults:")
    print(f"{'Dimension':<30} | {'QWK':>6} | {'Acc':>6} | {'F1w':>6} | {'F1m':>6}")
    print("-" * 70)
    qwks, accs, f1ws, f1ms = [], [], [], []
    for i, lbl in enumerate(CONFIG["labels"]):
        ...
        f1w = f1_score(t, p, labels=labels_range, average='weighted', zero_division=0)
        f1m = f1_score(t, p, labels=labels_range, average='macro', zero_division=0)
        metrics[lbl] = {"QWK": qwk, "Acc": acc, "F1_weighted": f1w, "F1_macro": f1m}
        qwks.append(qwk); accs.append(acc); f1ws.append(f1w); f1ms.append(f1m)
        print(f"{lbl:<30} | {qwk:6.3f} | {acc:6.3f} | {f1w:6.3f} | {f1m:6.3f}")

        cm = confusion_matrix(t, p, labels=labels_range)
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', ax=axes[i],
                    xticklabels=labels_range, yticklabels=labels_range)
        axes[i].set_title(f"{lbl}\nQWK: {qwk:.3f}  F1w: {f1w:.3f}")

    avg = {"QWK": float(np.mean(qwks)), "Acc": float(np.mean(accs)),
           "F1_weighted": float(np.mean(f1ws)), "F1_macro": float(np.mean(f1ms))}
    metrics["__avg__"] = avg
    print("-" * 70)
    print(f"{'AVERAGE':<30} | {avg['QWK']:6.3f} | {avg['Acc']:6.3f} | {avg['F1_weighted']:6.3f} | {avg['F1_macro']:6.3f}")
