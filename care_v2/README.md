...
## 🧠 Data Preprocessing & Retrieval Pipeline
The pipeline transforms raw dialogue into a rich, knowledge-enhanced representation:

1.  **Context Construction**: For every therapist utterance, the preceding $m=4$ turns (Therapist/Patient) are concatenated to form the conversational context.
2.  **Embedding Generation**: Utterances are embedded using **BGE-large-en-v1.5**. These embeddings are cached to speed up retrieval during training.
3.  **Prototype Label Pool**: A curated pool of therapist responses (categorized by dimension and score) serves as the "Knowledge Base." 
4.  **Retrieval Mechanism**: During training, the current utterance is used to retrieve $k$ similar positive and negative prototypes from the pool.
5.  **Explanation Integration**: Each retrieved prototype is paired with its **LLM-generated explanation** (from `faith_data/05_generated_explanations`). This provides the model with distilled reasoning for why that specific prototype received its score, acting as a form of expert guidance during training.
6.  **Fusion**: The context, current utterance, and retrieved explanations are fused using a cross-attention mechanism before being passed to the ordinal classification heads.
